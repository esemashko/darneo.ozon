...
BX.BitrixVue.component('ozon-product-list-item', {
    ...
    template: `
        ...
        <td v-html='item.ELEMENT_SECTION'></td>
        <td>
            <button class='btn btn-primary' type='button' v-on:click='showDataJson(item)'>
                <span>{{ $Bitrix.Loc.getMessage('DARNEO_OZON_VUE_PRODUCT_LOG_BUTTON_SHOW') }}</span>
            </button>
        </td>
        <!--<td>
            <div v-if='item.IS_FINISH'>
            ...
            <div v-else>
                <span class='badge badge-warning text-dark'>
                    {{ $Bitrix.Loc.getMessage('DARNEO_OZON_VUE_PRODUCT_LOG_UPDATE_STATUS') }}
                </span>
            </div>
        </td>-->
        </tr>
